...
static class GWREUtil{
    static public void TryChangeBase(Thing thing){
        ...
        if(sc.quality>=4 || thing.isReplica){
            ...
            thing.Say(text, thing, null, null);
            return;
        }
        bool success = false;
        int num = 120;
        ...
        if (source.offense.Length != 0){
            var dim = source.offense[1] * thing.material.dice / (num + (flag3 ? EClass.rnd(25) : 0));
        ...
        if (source.offense.Length > 2){
            var hit = sockethit + source.offense[2] * thing.material.atk * 9 / (num - (flag3 ? EClass.rnd(30) : 0));
        ...
        if (source.offense.Length > 3){
            ...
            var dmg = socketdmg + ench + source.offense[3] * thing.material.dmg * 5 / (num - (flag3 ? EClass.rnd(30) : 0));
            success |= SetBase(67, dmg);
        }

        if (source.defense.Length != 0)
            success |= SetBase(64, source.defense[0] * thing.material.dv * 7 / (num - (flag3 ? EClass.rnd(30) : 0)));
        ...
        if (source.defense.Length > 1){
            ...
            var pv = socketpv + ench + source.defense[1] * thing.material.pv * 9 / (num - (flag3 ? EClass.rnd(30) : 0));
        ...
        if(success){
            ...
            thing.Say(failText, thing, null, null);
        }
