...
public static class AttackProcess_CalcHit_Patch {
    public static bool Prefix(AttackProcess __instance, ref bool __result) {
        ...
        if ((float)(CC.Evalue(90) + ((__instance.weapon != null) ? __instance.weapon.Evalue(90, ignoreGlobalElement: true) : 0)) + Mathf.Sqrt(CC.Evalue(134)) > (float)EClass.rnd(200)) {
            __result = Crit();
            return false;
        }
        ...
        if (TC != null) {
            ...
            if (num != 0 && toHit < num * 10) {
                float num2 = evasion * 100 / Mathf.Clamp(toHit, 1, toHit);
